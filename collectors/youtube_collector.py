...
def search_videos(youtube, query: str, max_results: int, published_after: str) -> list[str]:
    ...
    try:
        ...
        if e.resp.status == 403:
            logger.error("YouTube quota exceeded — wait 24hrs")
            return []  # already in your code, just add the message
...
# collectors/youtube_collector.py
#
# UPDATED: Added quota exceeded error handling + transcript language fix.
#
# What changed vs original:
#   - search_videos(): added 403 quota check with clear error message
#   - get_transcript(): added "en-IN" language, removed "mr" (less common for Gujarat)
#   - get_comments(): added explicit textFormat="plainText" error logging
...
def build_queries(cfg: dict) -> dict[str, list[str]]:
    ...
    for region, langs in cfg["regions"].items():
        q_list = []
        for lang_terms in langs.values():
            for term in lang_terms[:3]:
                q_list.append(term)
        queries[region] = q_list[:6]
    return queries
...
def get_transcript(video_id: str, max_chars: int = 10000) -> str:
    ...
    UPDATED: Added "en-IN" language (common for Indian YouTube content).
    """
    try:
        from youtube_transcript_api import YouTubeTranscriptApi, TranscriptsDisabled, NoTranscriptFound
        transcript_list = YouTubeTranscriptApi.get_transcript(
            video_id,
            languages=["hi", "en", "en-IN", "gu"]   # ← UPDATED: added en-IN, removed mr
        )
        text = " ".join(seg["text"] for seg in transcript_list)
        logger.info(f"  ✓ Transcript fetched ({len(text)} chars) for {video_id}")
        return text[:max_chars]
...
def search_videos(youtube, query: str, max_results: int, published_after: str) -> list[str]:
    """
    Return video IDs.
    UPDATED: Added quota exceeded (403) detection with clear log message.
    """
    try:
        resp = youtube.search().list(
            q=query,
        ...
        # UPDATED: explicit quota exceeded message
        if e.resp.status == 403:
            logger.error(
                "YouTube API quota exceeded (403). "
                "You have used all 10,000 units for today. "
                "Quota resets at midnight Pacific Time. "
                "Stopping search to avoid further errors."
            )
            return []
        elif e.resp.status == 400:
            logger.error(f"YouTube search bad request (400) for query={query!r}: {e}")
            return []
        else:
            logger.error(f"YouTube search error ({e.resp.status}) for query={query!r}: {e}")
            return []
...
def get_video_details(youtube, video_ids: list[str]) -> list[dict]:
    ...
    try:
        ...
        # UPDATED: quota check here too
        if e.resp.status == 403:
            logger.error("YouTube API quota exceeded (403) during video detail fetch.")
        else:
            logger.error(f"YouTube video detail error: {e}")
        return []
...
def get_comments(youtube, video_id: str, max_results: int) -> list[dict]:
    try:
        ...
        if e.resp.status == 403:
            # UPDATED: differentiate quota vs comments-disabled
            reason = ""
            try:
            ...
            if reason == "quotaExceeded":
                logger.error("YouTube API quota exceeded (403) during comment fetch.")
            else:
                logger.debug(f"Comments disabled/forbidden for {video_id}: {e}")
        elif e.resp.status == 404:
            logger.debug(f"Video not found for comments: {video_id}")
        else:
            logger.error(f"Comment error for {video_id}: {e}")
        return []
